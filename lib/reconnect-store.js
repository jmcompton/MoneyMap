...
async function getReconnect(pool, { uid, scope, repId, filter } = {}) {
  ...
  if (!isMgr) {
    ...
    // Manager, no specific rep: scope to the manager's OWN company only. Never
    // all companies — that leaked demo/other-firm accounts into brand-new signups.
    params.push(uid);
    userFilter = `AND p.user_id IN (SELECT id FROM users WHERE company_id = (SELECT company_id FROM users WHERE id = $${params.length}))`;
  }

  // One row per prospect with its commission rollup + last-activity inputs.
  //
  // last_activity = GREATEST of REAL activity signals ONLY. We deliberately
  // EXCLUDE created_at and import-stamped last_activity_at: a commission import
  // is historical data, not a touch, so it must not reset the quiet clock. The
  // commission signal is the latest period treated as its MONTH-END date.
  // p.last_activity_at counts only when it's a genuine LATER touch (meaningfully
  // after created_at) — import inserts set both at once, so that pair is ignored.
  const sql = `
    WITH al AS (
      SELECT account_id,
    ...
    al_periods AS (
      SELECT account_id, COUNT(DISTINCT period_start) AS months_loaded
    ...
    SELECT
      ...
      COALESCE(alp.months_loaded, 0)             AS months_loaded,
      all_l.lines                                AS lines,
      (al.account_id IS NOT NULL)                AS has_commission,
      -- A real, substantive engagement exists (so empty/noise accounts drop out).
      (al.account_id IS NOT NULL
        OR lc.d IS NOT NULL OR ls.d IS NOT NULL
        OR lsmp.d IS NOT NULL OR lq.d IS NOT NULL) AS has_real_activity,
      GREATEST(
        -- latest commission period → that month's END date
        COALESCE((date_trunc('month', al.last_period) + INTERVAL '1 month - 1 day')::timestamptz, 'epoch'::timestamptz),
        ...
        -- genuine manual touch only (NOT the import stamp where touch≈created)
        COALESCE(CASE WHEN p.last_activity_at > p.created_at + INTERVAL '1 minute'
                      THEN p.last_activity_at END, 'epoch'::timestamptz)
      ) AS last_activity
      FROM prospects p
      LEFT JOIN users u            ON u.id = p.user_id
      LEFT JOIN al                 ON al.account_id = p.id
      LEFT JOIN al_periods alp     ON alp.account_id = p.id
  ...
  for (const r of rows) {
    ...
    // EXCLUDE accounts with no real activity at all (no lines/calls/visits/etc) —
    // empty or noise accounts must never surface in Reconnect.
    if (!r.has_real_activity) continue;

    const trailing = Number(r.trailing_commission) || 0;
    const months = Number(r.months_loaded) || 0;            // DISTINCT periods loaded
