...
const foundMoneyRoutes = require('./routes/foundmoney');
...
app.use(session({
  ...
  secret: process.env.SESSION_SECRET || 'change-me-set-SESSION_SECRET-env-var',
...
// Tenant context: every authenticated request carries its company_id so route
// handlers can scope queries by firm. req.companyId is the data wall — handlers
// filter on it (WHERE company_id = $req.companyId) so no firm sees another's data.
app.use((req, res, next) => {
  req.companyId = req.session.user ? req.session.user.companyId : null;
  res.locals.companyId = req.companyId;
  next();
});
...
app.use('/api/territory', requireAuth, territoryRoutes);
...
app.use('/api/found-money', requireAuthAPI, foundMoneyRoutes);
app.use('/api/tags', requireAuthAPI, tagRoutes);
