...
async function initDB() {
  await pool.query(`
    ...
    -- A "company" is one rep firm that pays for the product. Every user and
    -- every row of data belongs to exactly one company. All data queries filter
    -- by company_id, so no firm can ever see another firm's data. This table
    ...
    CREATE TABLE IF NOT EXISTS companies (
      ...
      name TEXT NOT NULL,
      slug TEXT UNIQUE,
      plan TEXT NOT NULL DEFAULT 'pilot',       -- pilot | paid | inactive
      status TEXT NOT NULL DEFAULT 'active',    -- active | suspended
      seat_limit INTEGER DEFAULT 10,            -- how many rep seats they've paid for
    ...
    -- ════════════════════════════════════════════════════════════
    -- Time Tracker — per-rep work session logging (generic, per company)
    -- ════════════════════════════════════════════════════════════
    ...
    -- Added as nullable IF NOT EXISTS columns so this is a safe, additive
    -- migration. Every data query in the route layer filters on this column;
    -- that filter is the wall between firms. (sessions = session store, so it
    -- is not stamped.)
    -- ════════════════════════════════════════════════════════════
    ALTER TABLE users                  ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ALTER TABLE prospects              ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ALTER TABLE calls                  ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ALTER TABLE quotes                 ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ALTER TABLE samples                ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ALTER TABLE calendar_events        ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ALTER TABLE brand_mappings         ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ALTER TABLE notifications          ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ALTER TABLE email_logs             ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ALTER TABLE onboarding_plans       ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ALTER TABLE weekly_plans           ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ALTER TABLE weekly_reports         ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ALTER TABLE time_sessions          ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ALTER TABLE import_history         ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ALTER TABLE commission_imports     ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ALTER TABLE commission_lines       ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ALTER TABLE commission_customer_map ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ALTER TABLE lines                  ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ALTER TABLE account_lines          ADD COLUMN IF NOT EXISTS company_id INTEGER REFERENCES companies(id) ON DELETE CASCADE;
    ...
    CREATE INDEX IF NOT EXISTS idx_commlines_company    ON commission_lines(company_id);
