...
router.get('/roster', async (req, res) => {
  try {
    ...
    const period_start = currentPeriodStart(period_type, req.query.period_start ? parseDate(req.query.period_start) : null);
    const { start, end } = periodBounds(period_type, period_start);
    const users = await pool.query('SELECT id, name, email, role FROM users WHERE company_id=$1 ORDER BY name ASC', [req.companyId]);
    const reps = [];
...
router.post('/generate', async (req, res) => {
  try {
    ...
    if (req.body.rep_id === 'all') {
      if (!isManager(me)) return res.status(403).json({ error: 'Forbidden' });
      const users = await pool.query('SELECT id, name FROM users WHERE company_id=$1', [req.companyId]);
      let n = 0;
      for (const u of users.rows) {
...
router.get('/export', async (req, res) => {
  try {
    ...
    if (scope === 'all') {
      if (!isManager(me)) return res.status(403).json({ error: 'Forbidden' });
      const users = await pool.query('SELECT id, name FROM users WHERE company_id=$1 ORDER BY name ASC', [req.companyId]);
      targets = users.rows;
    } else {
