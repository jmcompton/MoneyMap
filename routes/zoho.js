...
router.get('/users', async (req, res) => {
  try {
    const r = await pool.query('SELECT id, name, email FROM users WHERE company_id=$1 ORDER BY name', [req.companyId]);
    res.json(r.rows);
  } catch (e) {
...
router.post('/parse', async (req, res) => {
  try {
    ...
    const usersRes = await pool.query('SELECT id, name, email FROM users WHERE company_id=$1 ORDER BY name', [req.companyId]);
...
router.post('/import', async (req, res) => {
  ...
  const usersRes = await pool.query('SELECT id, name FROM users WHERE company_id=$1', [req.companyId]);
